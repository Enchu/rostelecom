...
const SearchModal = () => {
  ...
  const handleInputFocus = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement, Element>) => {
    e.target.classList.add('width_value')
  }
  ...
  const handleInputBlur = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement, Element>) => {
    ...
    e.target.classList.remove('width_value')
  }

  return (
    <div className={'search-modal'}>
      <button className="btn-reset search-modal__close" onClick={handleCloseSearchModal}/>
      <h3 className="search-modal__title">Поиск товаров</h3>
      <div className="search-modal__top">
        <label className="search-modal__label">
          <input type="text" className="search-modal__input" onFocus={handleInputFocus}/>
          <span className="search-modal__floating_label">{translations[lang].header.search_products}</span>
        </label>
      </div>
    </div>
  );
